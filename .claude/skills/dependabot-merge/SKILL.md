...
同じことを `.github/workflows/dependabot-review-release.yml` が毎週やります（レビューは
codex-fugu）。このスキルは、待たずに回したいときと、CI が open のまま残した PR を
片付けるときに使います。**判断基準は CI と揃えてあります。**
...
分かる情報は package.json 側に全部あります。

触っているファイルも確認します(`gh pr view <番号> --json files --jq '.files[].path'`)。
npm の更新なら package.json と pnpm-lock.yaml、GitHub Actions の更新なら
`.github/workflows/*.yml` しか動きません。**それ以外が混ざっていたら、その一点だけで
...
あなたが判断するのはセキュリティだけです。見るのは「この更新を取り込むと、脆弱性・
悪意あるコード・供給網の汚染を持ち込むか」だけ。API の互換性や機能の変化は判断材料では
ありません。破壊的変更・メジャーバージョンアップ・非推奨化・デフォルト値の変更は
どれも HOLD の理由になりません。見つけたら NOTES に書いて VERDICT は MERGE にすること。
互換性の問題は利用側のビルドや型検査で顕在化しますが、脆弱性は黙って通ります。
...
REASON: 1 行の理由
NOTES:
(セキュリティ以外で気づいたこと。破壊的変更・メジャーアップ・非推奨化・
　ライセンス変更など。判断には使わないが、利用側が知るべきなので必ず書く。
　無ければ「なし」)
DETAIL:
(確認内容。表にできるならする。確認できなかったことも書く)

HOLD にするのは次のいずれかに当てはまるときだけ。
...
セキュリティ上の疑いが晴れないなら HOLD。マージは後からできるが、悪意あるコードを
入れてから追うのは高くつく。逆に、セキュリティ上の懸念が無いなら MERGE。互換性を
理由に止めると、本来の目的であるセキュリティ修正の取り込みまで遅れる。
```
...
| PR | 更新内容 | 保留した理由 |
| --- | --- | --- |
| #15 | zod 3.25.76 -> 4.4.3 | 新バージョンが GHSA-xxxx-xxxx-xxxx の影響範囲に入る |
...
普段はワークフローが trusted publishing で publish します(README「npm への publish」)。
それは CI からしか効かないので、このスキルで手元から回したときは自分で上げます。
